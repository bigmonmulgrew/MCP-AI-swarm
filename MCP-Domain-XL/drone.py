...
class DomainDrone(BaseDroneServer):
    def _register_subclass_endpoints(self):
        ...
        async def handle_query(dqo: DroneQueryObject):
            ...
            prompt_preamble = f"Your task is to analyse the following domain data and provide the law and maintenance period in a JSON structured format ({json_format}):\n \nDomain Data:  "
            vector_results, chunks = get_vector_results(dqo.Query)
            neo4j_response = format_query_context(vector_results, chunks)
            
            # Extract confidence from results
            confidence_values = [
                {
                    "chunk_id": r.get("properties", {}).get("chunk_id"),
                    "similarity": r.get("similarity"),
                    "cleanliness": r.get("cleanliness"),
                    "final_confidence": r.get("confidence"),
                    "text_preview": r.get("properties", {}).get("text", "")[:200]
                }
            ...
            ai_payload = {
                ...
                # "prompt": f"""
                # Your task is to analyse the following domain data and provide the law and maintenance period in a JSON structured format ({json_format}):
                # Domain Data:  Law (Rule): All storage sites (of dangerous materials) must be constantly monitored\n\n
                
                # Maintenance Contract: 
                # CAMERA MAINTENANCE AGREEMENT
                ...
                # Name: _______________________________\nTitle: ________________________________\nSignature: ____________________________\nDate: ________________________________
                # """,
                "prompt": f"{prompt_preamble}  {domain_data}",
                "model": DOMAIN_MODEL,
                "options": {}
            }
        ...
        def get_vector_results(user_query):
            try:
            ...
            for result in results:
                ...
                chunk_vector = props.get("vector")
                chunk_cleanliness = props.get("cleanliness", 1.0)
                ...
                final_conf = 0.8 * sim + 0.2 * float(chunk_cleanliness)

                # attach confidence values to the result
                result["similarity"] = sim
                result["cleanliness"] = chunk_cleanliness
        ...
        def format_query_context(results, chunks):
            neo4j_response = []
            for result in results:
                text = result.get("properties", {}).get("text", "")
                conf = result.get("confidence", 0)
                if text:
                    neo4j_response.append(f"[confidence={conf:.2f}] {text}")
